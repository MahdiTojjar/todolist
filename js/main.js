...
add_btn.addEventListener('click', () => {
    if (task_input.value === '') {
        showAlertMessage('لطفاً یک کار وارد کنید', 'error');
    } else {
        addToDo(task_input);
        saveToLocalStorage();
        showAllTodos();
        task_input.value = '';
        showAlertMessage('کار با موفقیت اضافه شد', 'success');
    }
});
...
function deleteTodo(id) {
    ...
    saveToLocalStorage();
    showAlertMessage('کار با موفقیت حذف شد', 'success');
    showAllTodos();
}
...
function editTodo(id) {
    ...
    add_btn.addEventListener('click', () => {
        ...
        showAlertMessage('کار مورد نظر با موفقیت به روز شد', 'success');
    });
}
...
function clearAllTodos() {
    if(todos.length > 0) {
        todos = [];
        saveToLocalStorage();
        showAlertMessage('همه کارها با موفقیت پاک شد', 'success');
        showAllTodos();
    }else{
        showAlertMessage('هیچ کاری برای پاک کردن وجود ندارد', 'error');
    }
}